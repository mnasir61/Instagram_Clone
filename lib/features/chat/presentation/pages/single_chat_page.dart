...
class _SingleChatPageState extends State<SingleChatPage> {
  ...
  String _channelId = "";
  List<MyChatEntity> myChatEntity = [];
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<MyChatCubit, MyChatState>(
      builder: (context, state) {
        if (state is MyChatLoaded) {
          final message = state.myChat.first;
            ...
            child: StreamBuilder<List<UserEntity>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (snapshot.hasData) {
                    ...
                    final activeStatus = getActiveStatus(otherUserData, message);
                    return Scaffold(
                      appBar: AppBar(
                                      ...
                                      if (communicationState is CommunicationLoaded) {
                                        ...
                                        return ListView.builder(
                                          ...
                                          itemBuilder: (context, index) {
                                            ...
                                            return singleMessage.senderUid ==
                                                    widget.appEntity.senderUid
                                                ? GestureDetector(
                                                    onLongPress:(){
                                                      _deleteSingleMessage(singleMessage.messageId!);
  ...
  _sendTextMessage() async {
    if (_messageController.text.isEmpty) {
      ...
      final communicationCubit = BlocProvider.of<CommunicationCubit>(context);
      final getUsersCubit = BlocProvider.of<GetUsersCubit>(context);

      final textMessage = TextMessageEntity(
      ...
      final currentUser = UserEntity(lastActivity: _messageController.text);

      try {
        await communicationCubit.sendTextMessage(textMessage: textMessage, channelId: _channelId);
        await getUsersCubit.updateUser(user: currentUser);
        await communicationCubit.updateMyChat(myChatEntity: myChatEntity);
  ...
  //           .updateUser(user: UserEntity(lastActivity: _messageController.text));
  //     }).then((value) {
  //       BlocProvider.of<CommunicationCubit>(context).updateMyChat(
  ...
  _deleteSingleMessage(String messageId) {
    BlocProvider.of<CommunicationCubit>(context)
        .deleteSingleMessage(messageId: messageId);
  }
}
